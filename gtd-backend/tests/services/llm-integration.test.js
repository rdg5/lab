...
const llm_1 = require("@/services/llm"); // This will fail - doesn't exist yet
describe('LLM Integration Service', () => {
    ...
    beforeEach(async () => {
        ...
        llmService = new llm_1.LLMService(db, mockOpenAI, mockQueue, {
            model: 'gpt-4',
            maxTokens: 4000,
            temperature: 0.3,
    ...
    describe('Todo Analysis', () => {
        it('should analyze todo for GTD compliance', async () => {
            ...
            const mockAnalysis = factories_1.TestDataFactory.createLLMAnalysisResponse({
                clarified: true,
                qualityScore: 0.92,
                outcomeClarity: 0.95,
    ...
    describe('Background Processing', () => {
        it('should queue LLM analysis for background processing', async () => {
            ...
            expect(mockQueue.add).toHaveBeenCalledWith('llm-analysis', expect.objectContaining({
                todoId: todo.id,
                analysisType: 'quality_improvement',
            }), expect.objectContaining({
                priority: 'high',
        ...
        it('should process queued analysis jobs', async () => {
            ...
            const mockAnalysisResult = factories_1.TestDataFactory.createLLMAnalysisResponse();
            mockOpenAI.createChatCompletion.mockResolvedValueOnce({
                choices: [{
                        message: {
        ...
        it('should handle batch processing for multiple todos', async () => {
            ...
            const todos = [];
            for (let i = 0; i < 10; i++) {
                todos.push(factories_1.TestDataFactory.createTodo({ user_id: user.id }));
            }
            ...
            const mockBatchResult = todoIds.map(id => ({
                todoId: id,
                analysis: factories_1.TestDataFactory.createLLMAnalysisResponse(),
            }));
            mockOpenAI.createChatCompletion.mockResolvedValueOnce({
                choices: [{
                        message: {
        ...
        it('should handle rate limiting and retry logic', async () => {
            ...
            mockOpenAI.createChatCompletion
                ...
                choices: [{
                        message: {
                            content: JSON.stringify(factories_1.TestDataFactory.createLLMAnalysisResponse()),
                            role: 'assistant',
                        },
                        finish_reason: 'stop',
                    }],
    ...
    describe('Token Management', () => {
        ...
        it('should truncate content that exceeds token limits', async () => {
            ...
            const userMessage = actualCall.messages.find((m) => m.role === 'user');
