...
describe('Authentication API', () => {
    ...
    describe('OAuth Authentication', () => {
        describe('Google OAuth', () => {
            it('should authenticate user with valid Google OAuth code', async () => {
                const mockGoogleUser = factories_1.TestDataFactory.createGoogleOAuthUser();
                // Mock Google OAuth API response
                server.use(trpc.auth.googleOAuth.mutation((req, res, ctx) => {
                    return res(ctx.json({
                        user: {
            ...
            it('should handle invalid Google OAuth code', async () => {
                // This should fail - mutation doesn't exist
                await expect(trpc.auth.googleOAuth.mutate({
            ...
            it('should merge existing user on OAuth with same email', async () => {
                ...
                const existingUser = factories_1.TestDataFactory.createUser({
        ...
        describe('GitHub OAuth', () => {
            it('should authenticate user with valid GitHub OAuth code', async () => {
                const mockGitHubUser = factories_1.TestDataFactory.createGitHubOAuthUser();
                // This should fail - mutation doesn't exist
            ...
            it('should handle invalid GitHub OAuth code', async () => {
                await expect(trpc.auth.githubOAuth.mutate({
                    code: 'invalid-oauth-code',
                    state: 'some-state',
                })).rejects.toThrow('UNAUTHORIZED');
            });
    ...
    describe('JWT Token Management', () => {
        ...
        it('should reject invalid JWT tokens', async () => {
            await expect(trpc.auth.validateToken.query({
    ...
    describe('Authentication Middleware', () => {
        it('should protect authenticated routes', async () => {
            ...
            await expect(trpc.todos.list.query()).rejects.toThrow('UNAUTHORIZED');
        });
        ...
        it('should reject requests with malformed Authorization header', async () => {
            await expect(trpc.todos.list.query(undefined, {
                context: {
    ...
    describe('Rate Limiting', () => {
        it('should rate limit authentication attempts', async () => {
            ...
            for (let i = 0; i < 15; i++) {
                requests.push(trpc.auth.googleOAuth.mutate({
                    code: `attempt-${i}`,
            ...
            const rateLimitedErrors = results.filter(result => result instanceof server_1.TRPCError && result.code === 'TOO_MANY_REQUESTS');
        ...
        it('should have different rate limits for different endpoints', async () => {
            ...
            const oauthRequests = [];
            for (let i = 0; i < 10; i++) {
                oauthRequests.push(trpc.auth.googleOAuth.mutate({ code: `oauth-${i}` }).catch(err => err));
            }
            // Token validation should have more lenient limits
            const validationRequests = [];
            for (let i = 0; i < 50; i++) {
                validationRequests.push(trpc.auth.validateToken.query({ token: `token-${i}` }).catch(err => err));
            }
            ...
            const oauthRateLimited = oauthResults.filter(result => result instanceof server_1.TRPCError && result.code === 'TOO_MANY_REQUESTS');
            const validationRateLimited = validationResults.filter(result => result instanceof server_1.TRPCError && result.code === 'TOO_MANY_REQUESTS');
