...
describe('Todos API', () => {
  ...
  beforeEach(async () => {
    ...
    authUser = TestDataFactory.createUser();
  ...
  describe('Todo CRUD Operations', () => {
    describe('Create Todo', () => {
      ...
      it('should fail to create todo without outcome', async () => {
        ...
        await expect(
          trpc.todos.create.mutate(invalidTodoData)
      ...
      it('should automatically trigger LLM analysis for unclear todos', async () => {
        ...
        server.use(
          trpc.llm.analyzeTodo.mutation((req, res, ctx) => {
            return res(ctx.json({
              clarified: false,
              qualityScore: 0.3,
              suggestions: [
      ...
      it('should validate priority enum values', async () => {
        ...
        await expect(
          trpc.todos.create.mutate(todoData)
        ).rejects.toThrow('BAD_REQUEST');
      });
    ...
    describe('Read Todos', () => {
      it('should list todos for authenticated user', async () => {
        ...
        // Create test todos
        const todo1 = TestDataFactory.createTodo({ user_id: authUser.id });
        const todo2 = TestDataFactory.createTodo({ user_id: authUser.id });
        const otherUserTodo = TestDataFactory.createTodo(); // Different user
      ...
      it('should filter todos by completion status', async () => {
        ...
        const completedTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          completed: true 
        });
        const incompleteTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          completed: false 
        });
      ...
      it('should filter todos by priority', async () => {
        ...
        const highPriorityTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          priority: 'high' 
        });
        const lowPriorityTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          priority: 'low' 
        });
      ...
      it('should filter todos by context', async () => {
        ...
        const computerTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          context: '@computer' 
        });
        const callTodo = TestDataFactory.createTodo({ 
          user_id: authUser.id, 
          context: '@calls' 
        });
      ...
      it('should sort todos by due date', async () => {
        ...
        const todo1 = TestDataFactory.createTodo({ 
          user_id: authUser.id,
          due_date: new Date('2024-12-01')
        });
        const todo2 = TestDataFactory.createTodo({ 
          user_id: authUser.id,
      ...
      it('should get todo by ID', async () => {
        const db = dbManager.getKysely();
        const todo = TestDataFactory.createTodo({ user_id: authUser.id });
      ...
      it('should not allow accessing other users todos', async () => {
        ...
        await expect(
          trpc.todos.getById.query({ id: otherUserTodo.id })
    ...
    describe('Update Todo', () => {
      ...
      it('should update vector clock on modification', async () => {
        ...
        expect(updatedVectorClock['test-device-123']).toBeGreaterThan(
          originalVectorClock['test-device-123'] || 0
        );
      });
      ...
      it('should not allow updating other users todos', async () => {
        const db = dbManager.getKysely();
        const otherUserTodo = TestDataFactory.createTodo();
        await db.insertInto('todos').values(otherUserTodo).execute();
    ...
    describe('Delete Todo', () => {
      ...
      it('should not allow deleting other users todos', async () => {
        ...
        await expect(
          trpc.todos.delete.mutate({ id: otherUserTodo.id })
        ).rejects.toThrow('NOT_FOUND');
      });
    });
    ...
    describe('Complete/Uncomplete Todo', () => {
      ...
      it('should auto-complete parent todo when all subtasks completed', async () => {
        ...
        const subtask1 = TestDataFactory.createSubtask({ 
          todo_id: todo.id, 
          completed: true 
        });
        const subtask2 = TestDataFactory.createSubtask({ 
          todo_id: todo.id, 
          completed: false 
        });
  ...
  describe('GTD Quality Enforcement', () => {
    it('should reject todos below quality threshold', async () => {
      ...
      await expect(
        trpc.todos.create.mutate(poorQualityTodo)
      ).rejects.toThrow('BAD_REQUEST');
    });
    ...
    it('should validate outcome specificity', async () => {
      ...
      await expect(
        trpc.todos.create.mutate(vagueOutcomeTodo)
      ).rejects.toThrow('Outcome must be more specific');
    });
    ...
    it('should validate next action actionability', async () => {
      ...
      await expect(
        trpc.todos.create.mutate(vagueActionTodo)
      ).rejects.toThrow('Next action must be more specific');
    });
  });
