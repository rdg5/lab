...
import { RateLimitService } from '@/services/rate-limiting'; // This will fail - doesn't exist yet
...
describe('Rate Limiting Security', () => {
  ...
  describe('Authentication Rate Limiting', () => {
    it('should rate limit OAuth authentication attempts', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        requests.push(
          rateLimitService.checkLimit(clientIp, endpoint)
        );
      }
  ...
  describe('Security Attack Prevention', () => {
    ...
    it('should detect and block suspicious request patterns', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        rapidRequests.push(
          rateLimitService.checkLimit(clientIp, 'todos.list', {
