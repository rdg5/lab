...
import { ValidationService } from '@/services/validation'; // This will fail - doesn't exist yet
import { InputSanitizer } from '@/security/sanitizer'; // This will fail - doesn't exist yet
import { z } from 'zod';
  ...
  describe('Todo Input Validation', () => {
    ...
    it('should reject todos with invalid outcome', async () => {
      ...
      for (const outcome of invalidOutcomes) {
        ...
        const result = await validationService.validateTodoInput(todoData);
        expect(result.success).toBe(false);
        expect(result.error.issues.some((issue: any) => issue.path.includes('outcome'))).toBe(true);
      }
    });
    ...
    it('should reject todos with invalid next action', async () => {
      ...
      for (const nextAction of invalidActions) {
        ...
        const result = await validationService.validateTodoInput(todoData);
        expect(result.success).toBe(false);
        expect(result.error.issues.some((issue: any) => issue.path.includes('nextAction'))).toBe(true);
      }
    });
